...
class Kconfig(object):
    ...
    def __init__(
        ...
        try:
            ...
            log.note("Malformed KCONFIG_DEFAULTS_POLICY environment variable. Using default policy.")
    ...
    def load_config(
        ...
        **kwargs,
        ...
        if verbose is not None:
            log.warn(
                "Deprecation warning: load_config()'s 'verbose' argument has no effect. Since "
                "Kconfiglib 12.0.0, the message is returned from load_config() instead, "
                "and is always generated. Do e.g. print(kconf.load_config()) if you want to "
                "show a message like \"Loaded configuration '.config'\" on stdout."
            )
    ...
    def _load_config(self, filename, replace, load_deprecated, is_main_sdkconfig=True):
        ...
        choices_with_default_values: Set[Choice] = set()
        ...
        with self._open_config(filename) as f:
            if replace:
                ...
                # _was_set: If we're replacing the configuration, keep track of which
                ...
                # present_in_current_sdkconfig: In order to detect config options which are set multiple times
                #           in a single file, we must reset the present_in_current_sdkconfig flag for all symbols
                #           and choices every time we are loading the file.
                for sym in self.unique_defined_syms:
                    sym._was_set = False
            ...
            for linenr, line in enumerate(f, 1):
                ...
                if match:
                    ...
                    if (not sym or not sym.nodes) and not in_deprecated_block and self._deprecated_options:
                        ...
                        if new_name:
                            ...
                            if new_sym and new_sym.nodes:
                                ...
                                log.note(
                                    f"{escape(filename)}:{linenr}: {self.config_prefix + name} was replaced with "
                                    f"{self.config_prefix + new_name}"
                                    f"{' and inverted' if self._deprecated_options.is_inversion(name) else ''}"
                                )
                                sym = new_sym
                                name = new_name
                                value_is_default = False
                    ...
                    if sym.orig_type == BOOL:
                        ...
                        if not match:
                            log.warn(
                                f"{escape(filename)}:{linenr}: "
                                f"malformed string literal in assignment to {sym.name}. Assignment ignored."
                            )
                            value_is_default = False
                            continue
                    ...
                    if not match:
                        ...
                        if line and not line.lstrip().startswith("#"):
                            log.warn(f"{escape(filename)}:{linenr}: ignoring malformed line '{line}'")
                        continue
                ...
                # If value is supposed to be a default and symbol has a prompt, save it for later
                    ...
                    if not sym.choice:
                        ...
                        choices_with_default_values.add(sym.choice)
            ...
            for choice in choices_with_user_set_value:
                ...
                # is attempted to be set to n without setting another choice symbol to y
                    ...
                    if sel is not None:
                        log.note(
                            f"{escape(choice.nodes[0].filename)}:{choice.nodes[0].linenr}: "
            ...
            for choice in choices_with_default_values:
                choice.resolve_defaults()
        ...
        # Reset defaults_resolved flags to allow for next sdkconfig loading
        for sym in self.unique_defined_syms:
            sym._defaults_resolved = False
        for choice in self.unique_choices:
            choice._defaults_resolved = False
    ...
    def _undef_assign(self, name, val, filename, linenr):
        ...
        if self.warn_assign_undef:
            log.warn(f"{escape(filename)}:{linenr}: attempt to assign the value '{val}' to the undefined symbol {name}")
    ...
    def _assigned_twice(self, sym, new_val, filename, linenr):
        ...
        msg = f'{sym.name} set more than once. Old value "{user_val}", new value "{new_val}".'

        if user_val == new_val:
            if self.warn_assign_redun:
                log.warn(f"{escape(filename)}:{linenr}: {msg}")
    ...
    def _header_string(self, sym: "Symbol") -> str:
        ...
        if sym.orig_type == BOOL and val == "y":
            ...
            return f'#define {self.config_prefix}{sym.name} "{_escape(val)}"\n'
    ...
    def write_config(
        ...
        if verbose is not None:
            log.warn(
                "Deprecation warning: write_config()'s 'verbose' argument has no effect. Since "
                "Kconfiglib 12.0.0, the message is returned from write_config() instead, "
                "and is always generated. Do e.g. print(kconf.write_config()) if you want to "
                'show a message like "Saved configuration" on stdout.'
            )
            ...
            if match:
                ...
                if keyword:
                    ...
                    if token is not _T_CHOICE:
                        log.note(
                            f"{escape(self.filename)}:{self.linenr}: "
        ...
        while self._tokenize_line():
            ...
            if t0 in _TYPE_TOKENS:
                ...
                if node.item.orig_type and node.item.orig_type is not BOOL:  # type: ignore[union-attr]
                    ...
                    log.note(
                        f"{escape(self.filename)}:{node.linenr}: "
                ...
                if node.item and type(node.item) is Symbol:
                    log.note(
                        f"{escape(self.filename)}:{self.linenr}: "
                        f'config {node.item.name} {escape(_locs(node.item))} has a "visible if" option, '
                        "which is not supported for configs"
                    )
                elif node.item and type(node.item) is Choice:
                    log.note(
                        f"{escape(self.filename)}:{self.linenr}: "
                        f'choice {node.item.name} {escape(_locs(node.item))} has a "visible if" option, '  # type: ignore[union-attr]
                ...
                if node.warning:
                    log.note(
                        f"{escape(node.filename)}:{node.linenr}: "  # type: ignore[union-attr]
    ...
    def _sanitize_bool_literal_defaults(self, node: "MenuNode") -> None:
        ...
        for value, cond in node.defaults:
            if isinstance(value, Symbol) and value.name in INVALID_BOOL_LITERALS:
                log.note(
                    f"{escape(node.filename)}:{node.linenr}: "
                    f"{node.item.name}: 'default {value.name}' is not a valid bool value "  # type: ignore[union-attr]
    ...
    def _set_type(self, symbol_or_choice, new_type):
        ...
        if symbol_or_choice.orig_type and symbol_or_choice.orig_type != new_type:
            log.note(
                f"{escape(self.filename)}:{self.linenr}: "
                f"{symbol_or_choice.name} defined with multiple types, {TYPE_TO_STR[new_type]} will be used"
            )
    ...
    def _parse_prompt(self, node):
        ...
        if node.prompt:
            log.note(
                f"{escape(self.filename)}:{self.linenr}: "
                f"{node.item.name} defined with multiple prompts in single location"
            )
        ...
        if prompt != prompt.strip():
            log.note(
                f"{escape(self.filename)}:{self.linenr}: "
                f"{node.item.name} has leading or trailing whitespace in its prompt"
            )
    ...
    def _parse_help(self, node):
        if node.help is not None:
            log.note(
                f"{escape(self.filename)}:{self.linenr}: "
                f"{node.item.name} defined with more than one help text -- only the last one will be used"
            )
    ...
    def _empty_help(self, node, line):
        log.note(f"{escape(self.filename)}:{self.linenr}: {node.item.name} has 'help' but empty help text")
        node.help = ""
        if line:
            self._line_after_help(line)
    ...
    def _add_props_to_sym(self, node):
        ...
        if sym.warning and node.warning and sym.warning != node.warning:
            log.note(f"{escape(node.filename)}:{node.linenr}: {sym.name} defined with warnings in multiple locations.")
    ...
    def _check_multiple_definitions(self):
        ...
        for sym in self.unique_defined_syms:
            if len(sym.nodes) > 1:
                occurrences = {(os.path.abspath(node.filename), node.linenr) for node in sym.nodes}
        ...
        for choice in self.unique_choices:
            if len(choice.nodes) > 1 and choice.name not in self.allowed_multi_def_choices:
                occurrences = {(os.path.abspath(node.filename), node.linenr) for node in choice.nodes}
    ...
    def _check_sym_sanity(self):
        ...
        for sym in self.unique_defined_syms:
            sym_node = sym.nodes[0]
            loc = f"{escape(sym_node.filename)}:{sym_node.linenr}"
            ...
            if sym.orig_type == BOOL:
                ...
                for target_sym, _ in sym.selects:
                    if target_sym.orig_type not in _BOOL_UNKNOWN:
                        log.note(
                            f"{loc}: "
                            f"{escape(sym.name_and_loc)} selects the {TYPE_TO_STR[target_sym.orig_type]} symbol "
                            f"{target_sym.name}, which is not bool"
                        )
                ...
                for target_sym, _ in sym.implies:
                    if target_sym.orig_type not in _BOOL_UNKNOWN:
                        log.note(
                            f"{loc}: "
                            f"{sym.name} implies the {TYPE_TO_STR[target_sym.orig_type]} symbol "
                            f"{target_sym.name}, which is not bool"
                        )
                ...
                for default, _ in sym.defaults:
                    ...
                    if sym.orig_type == STRING:
                        ...
                        log.note(
                            f"{loc}: "
                            f"the {TYPE_TO_STR[sym.orig_type]} symbol {sym.name} has "
                            f"a non-{TYPE_TO_STR[sym.orig_type]} default {default.name}"
                        )
            ...
            if sym.ranges:
                if sym.orig_type not in _INT_HEX_FLOAT:
                    log.note(
                        f"{loc}: "
                        f"the {TYPE_TO_STR[sym.orig_type]} symbol {sym.name} "
                    ...
                    for low, high, _ in sym.ranges:
                        if not num_ok(low, sym.orig_type) or not num_ok(high, sym.orig_type):
                            log.note(
                                f"{loc}: "
                                f"the {TYPE_TO_STR[sym.orig_type]} symbol {sym.name} has "
                                f"a non-{TYPE_TO_STR[sym.orig_type]} range "
                                f"\\[{escape(low.name)}, {escape(high.name)}]"
                            )
    ...
    def _check_choice_sanity(self):
        ...
        def warn_select_imply(sym, expr, expr_type):
            sym_node = sym.nodes[0]
            sym_loc = f"{escape(sym_node.filename)}:{sym_node.linenr}"
            msg = (
                f"the choice symbol {sym.name} is {expr_type} by the following symbols, "
                "but select/imply has no effect on choice symbols"
            )

            # si = select/imply
            for si in split_expr(expr, OR):
                msg += "\n - " + split_expr(si, AND)[0].name

            log.note(f"{sym_loc}: {msg}")

        for choice in self.unique_choices:
            choice_node = choice.nodes[0]
            choice_loc = f"{escape(choice_node.filename)}:{choice_node.linenr}"

            if choice.orig_type != BOOL:
                log.note(f"{choice_loc}: {choice.name} defined with type {TYPE_TO_STR[choice.orig_type]}")

            for node in choice.nodes:
                if node.prompt:
                    break
            else:
                log.note(f"{choice_loc}: {choice.name} defined without a prompt")
            ...
            for default, _ in choice.defaults:
                ...
                if default.choice is not choice:
                    log.note(
                        f"{choice_loc}: "
                        f"the default selection {default.name} of {choice.name} "
            ...
            for sym in choice.syms:
                ...
                if sym.defaults:
                    log.note(
                        f"{sym_loc}: "
                        f"default on the choice symbol {sym.name} will have "
                ...
                for node in sym.nodes:
                    if node.parent.item is choice:
                        if not node.prompt:
                            log.note(
                                f"{escape(node.filename)}:{node.linenr}: the choice symbol {sym.name} has no prompt"
                            )

                    elif node.prompt:
                        log.note(
                            f"{escape(node.filename)}:{node.linenr}: "
                            f"the choice symbol {sym.name} is defined with a prompt outside the choice"
                        )
            ...
            if not sym.nodes and not is_num(sym.name):
                ...
                for node in self.node_iter():
                    if sym in node.referenced:
                        msg += f"\n\n- Referenced at {escape(node.filename)}:{node.linenr}:\n\n{node}"
                log.note(msg)
...
class Symbol:
    ...
    _defaults_resolved: bool
    _cached_vis: Optional[int]
    direct_dep: Union[Tuple, "Symbol"]
    ...
    def init_rest(self):
        ...
        """
        _defaults_resolved:
            Flag indicating whether the defaults have been resolved during sdkconfig loading.
        """
        self._defaults_resolved = False
    ...
    def _inject_default_value(self) -> bool:
        """
        Temporarily apply self._sdkconfig_value to the symbol as a first default value.
        ...
        if not self.value_is_valid(
            STR_TO_BOOL[self._sdkconfig_value] if self._sdkconfig_value in ("y", "n") else str(self._sdkconfig_value)
        ):
            log.note(
                f"'{self._sdkconfig_value}' is not a valid value for the "
                f"{TYPE_TO_STR[self.orig_type]} symbol {escape(self.name_and_loc)}. Assignment ignored."
            )
            return False
        ...
        parsing_kconfigs = self.kconfig._parsing_kconfigs
        self.kconfig._parsing_kconfigs = True
        # String literal is a constant symbol
        if self.orig_type == STRING:
            sym_for_val = self.kconfig._lookup_const_sym(self._sdkconfig_value or "")
        else:
            sym_for_val = (
                self.kconfig._lookup_const_sym(self._sdkconfig_value)
                if self._sdkconfig_value in ("y", "n")
                else self.kconfig._lookup_sym(self._sdkconfig_value)
            )
        ...
        dependency = self.kconfig.y
        for node in self.nodes:
            dependency = self.kconfig._make_and(dependency, node.dep)
        self.defaults = [(sym_for_val, dependency)]
        ...
        self._default_value_injected = True

        return True

    def resolve_defaults(self):
        """
        Decide whether there is a difference between sdkconfig and Kconfig default values.
        We cannot resolve it easily in place (self.str_value ?= sym._sdkconfig_value),
        because we need to consider all dependencies. See the example:

        Kconfig file:
        -------------
        config DEPENDENT
            string "Dependent symbol"
            default "string_1" if FREE=1
            default "string_2" if FREE=2
            default "fallback"
        ...
        --------------
        # default:
        CONFIG_DEPENDENT="string_1"
        # default:
        CONFIG_FREE=2

        In this case, Kconfig defaults are loaded (DEPENDENT="string_1", FREE=1).
        If we load sdkconfig line by line, by the time we load DEPENDENT, we would check whether
        sdkconfig default equals Kconfig default, which would be true, because we did not load FREE yet.
        We would then load FREE=2 (this mismatch would be correctly reported), but we would never return to DEPENDENT
        to check whether its default values between sdkconfig and Kconfig still match.

        In order to resolve defaults correctly, we need to go from free symbols (without any dependencies)
        to dependent symbols. However, because one config could depend on multiple other configs,
        we cannot (easily) use top-down approach.

        Instead, we resolve defaults for all symbols loaded as default and symbols themselves
        recursively ask those symbols they depend on to resolve their defaults first. In our example, DEPENDENT would
        first ask FREE to evaluate its default value and only after that, it would evaluate its own default value.
        """

        # defaults_resolved: we already resolved defaults for this symbol
        # _user_value is not None: user set the value (shouldn't happen, but just in case)
        # choice: it is a choice symbol, which is resolved on choice level
        # _sdkconfig_value is None: symbol is not present in sdkconfig (shouldn't happen, but just in case)
        # resolve_vis() == 0: symbol is not visible, so we don't need to resolve its defaults
        if (
            self._defaults_resolved
            or self._user_value is not None
            or self.choice
            or self._sdkconfig_value is None
            or self.resolve_vis() == 0
        ):
            return

        for sc in self.dependencies:
            # constant symbols have their fixed value and are free - no need to check them
            if sc.is_constant:
                continue
            sc.resolve_defaults()

        # Kconfig default value differs from sdkconfig default value
        if self.str_value != str(self._sdkconfig_value):
            self.kconfig.report.add_record(DefaultValuesArea, sym_or_choice=self)
            log.print(
                f"Default value for {self.name} in sdkconfig is "
                f"{ANSI_BOLD}{self.kconfig._quote_value(self._sdkconfig_value, self.orig_type)}{ANSI_END} "
                f"but it is {ANSI_BOLD}{self.kconfig._quote_value(self.str_value, self.orig_type)}{ANSI_END} "
                "according to Kconfig.",
                file=sys.stderr,
            )
            if self.kconfig.defaults_policy == DefaultsPolicy.USE_SDKCONFIG:  # Use default value from sdkconfig
                if self._inject_default_value():
                    log.print(
                    ...
                    log.print(
                        f"Failed to set default value for {self.name} from sdkconfig. "
                        "Default value will be set to "
                        f"{self.kconfig._quote_value(self.str_value, self.orig_type)}.",
                        file=sys.stderr,
                    )
            elif self.kconfig.defaults_policy == DefaultsPolicy.USE_KCONFIG:  # Use default value from Kconfig
                log.print(
                    "Using default value from Kconfig "
                    f"({ANSI_BOLD}{self.kconfig._quote_value(self.str_value, self.orig_type)}{ANSI_END}).",
                    file=sys.stderr,
                )
            elif self.kconfig.defaults_policy == DefaultsPolicy.INTERACTIVE:
                ...
                if preferred_source == "s":
                    ...
                    self.kconfig.set_value_and_source(self, self._sdkconfig_value, self.kconfig.filename)
                elif preferred_source == "k":
                    self.kconfig.set_value_and_source(self, self.str_value, self.kconfig.filename)
            else:
                raise KconfigError(
                    f"Unsupported default policy in KCONFIG_DEFAULTS_POLICY envvar: {self.kconfig.defaults_policy}"
                )

        self._defaults_resolved = True

    def resolve_vis(self) -> int:
        """
        Weaker version of resolve_defaults. We only want to know whether the symbol is visible or not,
        but we don't want to resolve its default value. Useful e.g. during choice default selection resolving.
        """
        for sc in self.dependencies:
            if sc == self.choice:  # possible transitive dependencies handled in choice's resolve_defaults()
                continue
            if sc.is_constant:
                # constant symbols have their fixed value and are free - no need to check them
                continue
        ...
        if self.orig_type in _INT_HEX:
            ...
            for rev_value in self.rev_values:
                ...
                if expr_value(cond):
                    if _is_base_n(candidate_val.name, base):
                        ...
                        num2str = str if base == 10 else hex
                        log.note(
                            f"indirectly set value {candidate_val.str_value} on "
                            f"{escape(self.name_and_loc)} (by {escape(src.name_and_loc)}) "
                            f"is not a valid base {base} number."
                        )
                    break
            else:
                self._has_active_indirect_set = False
            ...
            if vis and self._user_value and not self._has_active_indirect_set:
                ...
                if has_active_range and not low_int <= user_val <= high_int:
                    log.note(
                        ...
                        f"{escape(self.name_and_loc)} ignored due to being outside the active range "
                ...
                for weak_rev_value in self.weak_rev_values:
                    ...
                    if expr_value(cond):
                        if _is_base_n(candidate_val.name, base):
                            ...
                            num2str = str if base == 10 else hex
                            log.note(
                                f"indirectly set value {num2str(candidate_val)} on "
                                f"{escape(self.name_and_loc)} (by {escape(src.name_and_loc)}) "
                                f"is not a valid base {base} number."
                            )
                        break
            ...
            if has_active_range:
                ...
                if clamp is not None:
                    ...
                    if has_default:
                        num2str = str if base == 10 else hex
                        log.note(
                            f"default value {val_num_int} on {escape(self.name_and_loc)} clamped to "
                            f"{num2str(clamp)} due to being outside the active range "
                            f"([{num2str(low_int)}, {num2str(high_int)}])"
                        )
            ...
            for rev_value in self.rev_values:
                ...
                if expr_value(cond):
                    if is_float(candidate_val.name):
                        ...
                        log.note(
                            f"indirectly set value {candidate_val.str_value} on "
                            f"{escape(self.name_and_loc)} (by {escape(src.name_and_loc)}) is not a valid float."
                        )
                    break
            else:
                self._has_active_indirect_set = False
            ...
            if has_active_range:
                ...
                if float_clamp is not None:
                    ...
                    if has_default:
                        log.note(
                            f"default value {val_num_float} on {escape(self.name_and_loc)} clamped to "
                            f"{float_clamp} due to being outside the active range "
                            f"([{low_float}, {high_float}])"
                        )
    ...
    def bool_value(self) -> int:
        ...
        if self.orig_type != BOOL:
            ...
            if self.orig_type:  # != UNKNOWN
                ...
                log.note(
                    f"The {TYPE_TO_STR[self.orig_type]} symbol {escape(self.name_and_loc)} is being evaluated in a "
                    "logical context somewhere. It will always evaluate to n."
                )
    ...
    def config_string(self):
        ...
        # sym.orig_type == STRING
        return f'{pragma_default_comment}{self.kconfig.config_prefix}{self.name}="{_escape(val)}"\n'
    ...
    @property
    def referenced(self) -> Union[Set["Symbol"], Set["Choice"]]:
        """
    ...
    @property
    def dependencies(self) -> Set[Union["Symbol", "Choice"]]:
        """
        A set() with all symbols and choices that this symbol depends on.

        Unlike 'referenced', this excludes symbols that are targets of 'select' and 'imply'
        statements, as those represent reverse dependencies (things this symbol affects)
        rather than true dependencies (things that affect this symbol's value/visibility).

        See MenuNode.dependencies for more details.
        """
        return {item for node in self.nodes for item in node.dependencies}
    ...
    def _rec_invalidate_if_has_prompt(self):
        ...
        if self.kconfig._warn_assign_no_prompt:
            log.note(f"{escape(self.name_and_loc)} has no prompt, meaning user values have no effect on it")
    ...
    def _warn_select_unsatisfied_deps(self):
        ...
        msg = (
            f"{escape(self.name_and_loc)} has direct dependencies {expr_str(self.direct_dep)} with value "
        ...
        for select in split_expr(self.rev_dep, OR):
            ...
            msg += (
                f"\n - {escape(selecting_sym.name_and_loc)}, with value {selecting_sym.str_value}, direct dependencies "
...
class Choice:
    ...
    def resolve_defaults(self):
        ...
        if self._defaults_resolved or self.resolve_vis() == 0:
            return

        if self._user_selection is not None:
            for sym in self.syms:
                self.kconfig.set_value_and_source(sym, sym.bool_value, self.kconfig.filename)
            # In this case, we did not resolve "defaults", but the flag can still be used.
            self._defaults_resolved = True
            return

        sc: Union[Symbol, Choice]
        for sc in self.dependencies:
            if sc.is_constant:
                # constant symbols have their fixed value and are free - no need to check them
                continue
        ...
        # syms that should be y according to sdkconfig and are visible
        # NOTE: This is tricky; we dont want to resolve sym's value here,
        # but we need to know whether the symbol is visible or not.
        y_syms_from_sdkconfig = [sym for sym in self.syms if sym._sdkconfig_value == "y" and sym.resolve_vis() == 2]

        for sym in self.syms:
            if sym._sdkconfig_value == "y" and sym.visibility == 0:
                log.note(
                    f"{escape(self.nodes[0].filename)}:{self.nodes[0].linenr}: "
                    f"Choice {self.name} has {sym.name} as default selection, but given symbol is not visible. "
                    "Ignoring."
                    if self.nodes
                    else (
                        f"Choice {self.name} has {sym.name} as default selection, but given symbol is not visible. "
        ...
        if len(y_syms_from_sdkconfig) == 1:
            ...
            diff = [
                sym
                for sym in self.syms
                if sym.str_value != sym._sdkconfig_value and sym._sdkconfig_value is not None and sym._loaded_as_default
            ]
            ...
            if len(diff) > 0:
                ...
                if self.kconfig.defaults_policy == DefaultsPolicy.USE_SDKCONFIG:
                    self._inject_default_value_for_choice(sdkconfig_selection)
                elif self.kconfig.defaults_policy == DefaultsPolicy.INTERACTIVE:
                    self._handle_interactive_choice()
            ...
            if self.kconfig.defaults_policy == DefaultsPolicy.USE_SDKCONFIG:
                self._inject_default_value_for_choice(selection)
            elif self.kconfig.defaults_policy == DefaultsPolicy.INTERACTIVE:
                self._handle_interactive_choice()
        else:
            if self.visibility:
                _file = self.nodes[0].filename if self.nodes else None
                _line = self.nodes[0].linenr if self.nodes else None
                if self.selection:
                    log.note(
                        f"{_file}:{_line}: "
                        f"Choice {self.name} has no valid sdkconfig selection set to y. "
                        f"Will use Kconfig default selection {self.selection.name}."
                        if _file is not None and _line is not None
                        else (
                            f"Choice {self.name} has no valid sdkconfig selection set to y. "
                            f"Will use Kconfig default selection {self.selection.name}."
                        )
                    ...
                    log.note(
                        f"{_file}:{_line}: "
                        f"Choice {self.name} has no sdkconfig nor kconfig selection set to y. "
                        f"Choice will be deselected."
                        if _file is not None and _line is not None
                        else (
                            f"Choice {self.name} has no sdkconfig nor kconfig selection set to y. "
                            f"Choice will be deselected."
            ...
            # else: silently ignore: invisible choice should have no y-selected symbols, so it is OK

        self._defaults_resolved = True

    def _inject_default_value_for_choice(self, sym: Symbol) -> None:
        """
        ...
        if sym not in self.syms:
            log.note(
                f"{escape(self.nodes[0].filename)}:{self.nodes[0].linenr}: "
                f"Attempted to select symbol {sym.name} in choice {self.name}, "
    ...
    def _handle_interactive_choice(self) -> Tuple[Dict[str, Tuple[str, str]], Dict[str, str]]:
        ...
        while True:
            ...
            if char in selections:
                self.kconfig.set_value_and_source(self.syms[int(char) - 1], "y", self.kconfig.filename)
                changed_symbols = {
                ...
                unchanged_symbols = {
                    sym.name: sym.str_value for sym in self.syms if sym.str_value == sym._sdkconfig_value
                }
                break
    ...
    def visibility(self) -> int:
        ...
        """
        if self._cached_vis is None:
            self._cached_vis = _visibility(self)  # type: ignore
        return self._cached_vis  # type: ignore
    ...
    @property
    def referenced(self) -> Set[Union["Symbol", "Choice"]]:
        """
        See the class documentation.
        """
        return {item for node in self.nodes for item in node.referenced}
    ...
    def dependencies(self) -> Set[Union["Symbol", "Choice"]]:
        """
        See the class documentation and MenuNode.dependencies.
        """
        return {item for node in self.nodes for item in node.dependencies}
...
class MenuNode:
    ...
    def __init__(
        ...
        # Cached dependencies set, computed on first access
        self._cached_dependees = None
    ...
    def dependencies(self):
        """
        A set() with all symbols and choices that this menu node depends on.
        ...
        Includes:
        - Symbols from 'depends on' (self.dep)
        - Symbols from prompt conditions
        - Symbols from default value expressions and conditions
        - Symbols from select/imply conditions (but NOT the targets)
        - Symbols from range expressions and conditions
        - For MENU items: symbols from visibility
        """
        if self._cached_dependees is not None:
            return self._cached_dependees
        ...
        # For selects and implies, only include the condition, not the target.
        # The target is what we're affecting (reverse dependency), not depending on.
        for _target, cond in self.selects:
            res |= expr_items(cond)

        for _target, cond in self.implies:
            res |= expr_items(cond)
    ...
    def _sym_choice_node_str(self, sc_expr_str_fn):
        ...
        if self.prompt:
            ...
            indent_add_cond(prefix + f' "{_escape(self.prompt[0])}"', self.orig_prompt[1])
...
def _info_fn(kconf, _, msg):
    log.note(f"{escape(kconf.filename)}:{kconf.linenr}: {msg}")
...
def _warning_if_fn(kconf, _, cond, msg):
    if cond == "y":
        log.warn(f"{escape(kconf.filename)}:{kconf.linenr}: {msg}")
...
def _shell_fn(kconf, _, command):
    ...
    if stderr:
        log.warn(
            f"{escape(kconf.filename)}:{kconf.linenr}: '{{}}' wrote to stderr: {{}}".format(
